...
# Fixed-layout manifest for the native (C) enumerator, so it never has to parse
# JSON.  Little-endian; the 44-byte header is followed by three variable arrays:
#   alpha_code_lens : alpha_size x u8   (UTF-8 byte length of each alphabet char)
#   alpha_utf8      : alpha_bytes       (concatenated UTF-8 of the alphabet)
#   ln_levels       : ln_len x u8
# Header: magic(4s) version ngram levels max_length ep_enabled alpha_size ln_len
#         alpha_bytes (8x u32) then lam (f64).
MANIFEST_MAGIC = b"OMN1"
_MANIFEST_HEADER = struct.Struct("<4s8Id")
...
class NgramModel:
    ...
    def save(self, directory: Path | str) -> None:
        ...
        (path / MANIFEST_NAME).write_bytes(self._manifest_bytes())

    def _manifest_bytes(self) -> bytes:
        """Serialise the fixed-layout manifest consumed by the native enumerator."""
        encoded = [ch.encode("utf-8") for ch in self.alphabet.chars]
        code_lens = bytes(len(e) for e in encoded)
        alpha_utf8 = b"".join(encoded)
        ln = bytes(self.ln_levels)
        header = _MANIFEST_HEADER.pack(
            ...
            self.scale.levels,
            self.max_length,
            1 if self.ep_enabled else 0,
            self.alphabet_size,
            len(self.ln_levels),
            len(alpha_utf8),
            self.scale.lam,
        )
        return header + code_lens + alpha_utf8 + ln
